            ...
            <div class="col-12">
                <div class="card mb-4 mx-4">
                    <div class="card-header pb-0">
                            ...
                            <div>
                                <h5 class="mb-0">Tool Groups</h5>
                            </div>
                            <a href="{{ route('Tool-Group.create') }}" class="btn bg-gradient-primary btn-sm mb-0" type="button">+&nbsp; New Group</a>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @foreach ($ToolGroups as $group)
                                        <tr>
                                            ...
                                            <td>
                                                <p class="text-xs font-weight-bold mb-0">{{ $group->descricao }}</p>
                                            </td>
                                            <td class="align-middle text-center">
                                                <span class="text-secondary text-xs font-weight-bold">{{ $group->tools->count() }}</span>
                                            </td>
                                            <td class="align-middle text-center">
...
<!-- Modal -->
<div wire:ignore.self class="modal fade" id="toolGroupModal" tabindex="-1" aria-labelledby="toolGroupModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="toolGroupModalLabel">{{ $editingId ? 'Edit Tool Group' : 'New Tool Group' }}</h5>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const modalEl = document.getElementById('toolGroupModal');
        if (!modalEl) return;
        const bsModal = new bootstrap.Modal(modalEl);

        window.addEventListener('show-tool-group-modal', () => {
            bsModal.show();
        });
        window.addEventListener('hide-tool-group-modal', () => {
            bsModal.hide();
        });
    });
</script>
